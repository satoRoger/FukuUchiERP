...
export default async function GetEvents(
  ...
  const employee = query.userId
    ? new EmployeeFactory().create(query.userId)
    : undefined;
  const facility = query.facilityId
    ? new FacilityFactory().create(query.facilityId)
    : undefined;
  ...
  return collection
    .getData()
    .map((event) => EventAPIInterface.fromDomainEvent(event));
}
