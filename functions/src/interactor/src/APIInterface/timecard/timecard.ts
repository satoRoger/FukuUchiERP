...
import { isCardType, isCoordinate } from '../../../../domain/attendanceManagement/src/service/utility/typeGuard';
import { DateTime } from 'luxon';
import { isISOString, isDateTime, isString } from '../../../../util/isType/isType';
...
export default class TimecardAPIInterface {
	...
	constructor(id: any, cardType: any, userId: any, date: any, coordinate?: any) {
		if (isString(id)) {
			this.id = id;
		} else {
			throw '';
		...
		if (isISOString(date)) {
			...
			this.date = date.toISO({ includeOffset: false });
		} else {
			throw '';
		}
