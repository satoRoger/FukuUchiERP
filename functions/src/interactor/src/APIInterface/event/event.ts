...
import { isISOString, isString, isEventType, isStringArray, isDateTime } from '../../../../util/isType/isType';
...
export default class EventAPIInterface {
  ...
  static fromDomainEvent(event: CalendarEvent) {
	...
	constructor(id: any, start: any, end: any, title: any, type: any, userId?: any, facilityId?: any) {
		if (isString(id)) {
			this.id = id;
		} else {
			throw '';
		...
		if (isISOString(start)) {
			...
			this.start = start.toISO({ includeOffset: false };
		} else {
			throw '';
		}
		if (isISOString(end)) {
			...
			this.end = end.toISO({ includeOffset: false };
		} else {
			throw '';
		}
		if (isString(title)) {
