...
export default class FireWorkflowsSearch {
	...
	constructor(
		...
		this.facilityId = option.facilityId ? new FireFacilityRefference(connectionDB, option.facilityId) : undefined;
	}
	searchRepository(): QueryRepository {
		let queryRepository: QueryRepository = this.connectionDB.collection(CollectionName.timecards);

		if (this.facilityId) {
			queryRepository = queryRepository.where(UsersProperty.facilityId, '==', this.facilityId.toFireStore());
		}
