...
export default class EventRepositoryFS implements EventRepository {
	...
	async save(event: CalendarEvent): Promise<CalendarEvent> {
		const userRef = event.employeeId ? this.database.collection('users').doc(event.employeeId.value) : null;
	...
	async search(from?: DateTime, to?: DateTime, employee?: employee, facility?: facility): Promise<EventCollection> {
		let queryRepository:
			| FirebaseFirestore.CollectionReference<FirebaseFirestore.DocumentData>
			| FirebaseFirestore.Query<FirebaseFirestore.DocumentData> = this.repository;
		...
		if (facility) {
			const facilityRef = this.database.collection('facilities').doc(facility.id.value);
			queryRepository = queryRepository.where('facilityIds', 'array-contains', facilityRef);
		}
		if (from) {
		...
		const result = snapshot.docs.map(async (doc) => {
			...
			const facilityDocId = data.facilityId ? await data.facilityId.get().id : undefined;
